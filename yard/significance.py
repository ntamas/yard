...
class SignificanceTest(object):
    ...
    def test(data1, data2):
        ...
        `yard.data.BinaryClassifierData`. Returns the p-value."""
...
class PairedPermutationTest(object):
    ...
    def test(data1, data2):
        ...
        observed_diff = abs(auc_from_ranks(ranks1) -
                            auc_from_ranks(ranks2))
        num_success = 0
        ...
        for trial in xrange(self.num_repetitions):
            ...
            diff = abs(auc_from_ranks(ranks1) - auc_from_ranks(ranks2))
            if diff >= observed_diff:
                num_success += 1

        return num_success / float(self.num_repetitions)
